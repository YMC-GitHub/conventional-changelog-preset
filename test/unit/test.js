...
describe('yemiancheng preset', function() {
  this.timeout(30000);
  it('should work if there is no semver tag', function(done) {
    // only need the setup 1
    preparing(1);
  ...
  it.skip('should replace #[0-9]+ with GitHub issue URL', function(done) {
    // only need the setup 1 and 2
    preparing(2);
    ...
    conventionalChangelogCore({
      ...
      .pipe(
        through(function(chunk) {
          chunk = chunk.toString();
          expect(chunk).to.include('feat(awesome): addresses the issue brought up in #133');
          done();
        })
      );
  });

  it.skip('should remove the issues that already appear in the subject', function(done) {
    preparing(3);
  ...
  it.skip('should fallback to the closest package.json when not providing a location for a package.json', function(done) {
    preparing(7);
    let i = 0;
  ...
  it.skip('should only replace with link to user if it is an username', function(done) {
    preparing(8);
  ...
  it.skip('parses both default (Revert "<subject>") and custom (revert: <subject>) revert commits', function(done) {
    preparing(9);
